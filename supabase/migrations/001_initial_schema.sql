...
CREATE TABLE participant_statuses (
  ...
  forfeit_match_id UUID REFERENCES matches(id) ON DELETE SET NULL,
  original_match_state JSONB,
  promoted_opponent_id UUID REFERENCES participants(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
